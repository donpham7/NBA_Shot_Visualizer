...
class CourtCoordinates:
    ...
    def __init__(self):
        ...
        self.hoop_radius = 0.75
        self.three_arc_distance = 23.75  # the NCAA men's three arc is 23ft and 9in from the center of the hoop
        self.three_straight_distance = 22.1  # the NCAA men's three straight section is 22ft 3in from the center of the hoop
        self.three_straight_length = (
            13.1666  # the NBA men's three straight section = 13 ft and 2in
        )
    ...
    def __get_three_point_coordinates(self, loc):
        ...
        start_straight = [[strt_dst_start, 0, 0], [strt_dst_start, strt_len, 0]]
        end_straight = [[strt_dst_end, strt_len, 0], [strt_dst_end, 0, 0]]
        line_coordinates = []
        ...
        if loc == "near":
            ...
            start_straight = [
                [strt_dst_start, crt_len, 0],
                [strt_dst_start, crt_len - strt_len, 0],
            ]
            end_straight = [
                [strt_dst_end, crt_len - strt_len, 0],
                [strt_dst_end, crt_len, 0],
            ]
        ...
        for x_coord in np.linspace(
            int(strt_dst_start) + 0.9, int(strt_dst_end) - 0.1, 100
        ):
    ...
    def __get_hoop_coordinates(self, loc):
        ...
        hoop_loc_x, hoop_loc_y, hoop_loc_z = (
            self.hoop_loc_x,
            self.hoop_loc_y,
            self.hoop_loc_z,
        )
        ...
        a = 1
        b = -2 * hoop_loc_y
        for hoop_coord_x in np.arange(
            hoop_min_x, hoop_max_x + hoop_step / 2, hoop_step
        ):
            c = (
                hoop_loc_y**2
                + (hoop_loc_x - round(hoop_coord_x, 2)) ** 2
                - hoop_radius**2
            )
            ...
            hoop_coordinates_bottom_half.append(
                [hoop_coord_x, hoop_coord_y2, hoop_loc_z]
            )

        hoop_coordinates_df = pd.DataFrame(
            hoop_coordinates_top_half + hoop_coordinates_bottom_half[::-1],
            columns=["x", "y", "z"],
        )
